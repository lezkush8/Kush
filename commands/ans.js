...
zokou({nomCom:"bot",reaction:"💀",categorie:"IA"},async(dest,zk,commandeOptions)=>{
  ...
  const {repondre,ms,arg}=commandeOptions;
    ...
    if(!arg || !arg[0])
    {return repondre("im listening to you...!.")}
    //var quest = arg.join(' ');
  try{
...
.catch(error => {
  ...
  zokou({ nomCom: "kush", reaction: "💯", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    const { repondre, arg, ms } = commandeOptions;
...
zokou({ nomCom: "lazkush", reaction: "🙋", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    const { repondre, arg, ms } = commandeOptions;
