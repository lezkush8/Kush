...
setInterval(async () => {
    if (conf.AUTO_BIO === "yes") {
        ...
        const bioText = `lezkush tech is online!\n${currentDateTime}`; // Format the bio text
...
// Other functions (auto-react, anti-delete, etc.) as needed
  ...
  if (conf.ANTICALL === 'yes') {
    ...
    await zk.sendMessage(callerId, {
      text: "Am lezkush tech,, My owner is unavailable try again later"
    });
  }
});
...
const {
    ...
    zk.ev.on('group-participants.update', async group => {
      ...
      try {
        ...
        ppgroup = 'https://files.catbox.moe/o2pgsf.jpg';
      }
      try {
        const metadata = await zk.groupMetadata(group.id);
        if (group.action == 'add' && (await recupevents(group.id, "welcome")) == 'on') {
          let msg = `lezkush md welcome message
`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                ...
                console.log("✅ lezkush Connected to WhatsApp! ☺️");
                console.log("--");
                await (0, baileys_1.delay)(200);
                console.log("------");
                await (0, baileys_1.delay)(300);
                console.log("------------------/-----");
                console.log("lezkush Md is Online 🕸\n\n");
